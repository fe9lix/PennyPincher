...
class SwiftPennyPincherTests: XCTestCase {
    ...
    func testInvalidRecognition() {
        if let (foundTemplate, similarity) = PennyPincher.recognize([CGPoint](), templates: [PennyPincherTemplate]()) {
    ...
    func testTemplateRecognition() {
        ...
        if let (foundTemplate, similarity) = PennyPincher.recognize(points, templates: templates) {
    ...
    func testExactRecognition() {
        ...
        if let template = template, (foundTemplate, similarity) = PennyPincher.recognize(points, templates: [template]) {
            XCTAssertEqual(foundTemplate.id, template.id)
            XCTAssertEqualWithAccuracy(similarity, 12.0, 0.1)
        } else {
            XCTFail("Expected recognition result.")
        }
    }
    ...
    func testPerformanceExample() {
        var points = templatePoints()
        var templates = randomTemplates(1000)
